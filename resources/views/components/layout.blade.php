...
<div class="min-h-screen grid grid-rows-[auto,1fr,auto] font-arial">
    <header class="text-xl font-bold text-white bg-gray-600"><div class="mx-auto container p-2 grid grid-cols-[1fr,auto]">
            <div>BotBuddy</div>
            @if(auth()->check() && auth()->user()->email_verified_at)<div class="md:hidden" id="toggle">☰</div>@endif
        </div>
    </header>
    @if(auth()->check() && auth()->user()->email_verified_at)
    <main class="min-h-full grid gap-2 grid-rows-[auto,1fr] md:grid-rows-[auto] md:grid-cols-[auto,1fr] md:mx-auto md:container">
        <x-nav />
