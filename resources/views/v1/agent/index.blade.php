            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <div class="font-bold text-gray-900 dark:text-white">Agents</div>
                    </div>
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <a href="{{ route('agent.create') }}" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tr class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <a href="{{ route('agent.show', $agent->id) }}">{{ $agent->name }}</a>
                            </th>
                            <td class="px-4 py-3">{{ $agent->client_type }}</td>
{{--                            <td class="px-4 py-3">{{ $agent->uuid }}</td>--}}
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                <button id="script-{{ $agent->id }}-dropdown-button" data-dropdown-toggle="script-{{ $agent->id }}-dropdown" class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                    </svg>
                                </button>
                                <div id="script-{{ $agent->id }}-dropdown" class="mt-[8.5rem] mr-[-1rem] hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="script-{{ $agent->id }}-dropdown-button">
                                        <li>
                                            <a href="{{ route('agent.show', $agent->id) }}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                        </li>
                                    </ul>
                                    <div class="py-1">
                    ...
                    <div class="flex-grow">
                        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-white">
                                {{ ($agents->currentPage() - 1) * $agents->perPage() + 1 }}-{{ min($agents->total(), $agents->currentPage() * $agents->perPage()) }}
                            </span>
                            of
                            <span class="font-semibold text-gray-900 dark:text-white">{{ $agents->total() }}</span>
                        </span>
                    </div>
                    <nav>
