<x-v1.layout page="Delete {{ $group->name }}">
    ...
    <div class="max-w-2xl px-4 mx-auto">
        <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Delete proxy group</h2>
        <form method="post" action="{{ route('proxy.group.destroy', $group->id) }}">
            @csrf
            ...
            <div class="grid gap-4 mb-4 sm:grid-cols-2 sm:gap-6 sm:mb-5">
                <div class="sm:col-span-2 text-gray-900 dark:text-white">
                    ...
                    @if($group->proxies_count > 0 || $totalAccounts > 0)
                        <p>Deleting this proxy group will also perform the following:</p>
                        <ul>
                            <li>- Delete {{ $group->proxies_count }} proxies</li>
                            <li>- Unassign proxies from {{ $totalAccounts }} accounts</li>
                        </ul>
                    @endif
                </div>
            </div>
