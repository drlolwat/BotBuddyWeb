            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <div class="font-bold text-gray-900 dark:text-white">Proxy Groups</div>
                    </div>
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <a href="{{ route('proxy.group.create') }}" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                        ...
                        @foreach($proxyGroups as $proxyGroup)
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    ...
                                    <div id="proxy-{{ $proxyGroup->id }}-dropdown" class="mt-[8.5rem] mr-[-1rem] hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="proxy-{{ $proxyGroup->id }}-dropdown-button">
                                            <li>
                                                <a href="{{ route('proxy.group.show', $proxyGroup->id) }}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                            </li>
                                        </ul>
                                        <div class="py-1">
                                            <form method="post" action="{{ route('proxy.group.destroy', $proxyGroup->id) }}">
                                                @csrf
                    ...
                    <div class="flex-grow">
                        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-white">
                                {{ ($proxyGroups->currentPage() - 1) * $proxyGroups->perPage() + 1 }}-{{ min($proxyGroups->total(), $proxyGroups->currentPage() * $proxyGroups->perPage()) }}
                            </span>
                            of
                            <span class="font-semibold text-gray-900 dark:text-white">{{ $proxyGroups->total() }}</span>
                        </span>
                    </div>
                    <nav>
                        <ul class="inline-flex -space-x-px text-sm">
                            @php
                                $currentPage = $proxyGroups->currentPage();
                                $lastPage = $proxyGroups->lastPage();
                                $pages = [];
