<x-v1.layout page="Account Groups">
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    ...
                                    <div id="account-{{ $accountGroup->id }}-dropdown" class="mt-[14.5rem] mr-[-1rem] hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="account-{{ $accountGroup->id }}-dropdown-button">
                                            ...
                                            @endif
                                            <li>
                                                <a id="change-proxy-{{ $accountGroup->id }}" data-modal-target="change-proxy-modal" data-modal-toggle="change-proxy-modal" class="cursor-pointer block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Change proxies</a>
                                            </li>
                                            <form method="post" action="{{ route('account.group.remove_proxy', $accountGroup->id) }}">@csrf
                                                <button class="w-full text-left block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Remove proxies</button>
                                            </form>
                                        </ul>
                                        <div class="py-1">
                                            <div>
    ...
    <div id="change-proxy-modal" tabindex="-1" aria-hidden="true" class="hidden bg-[#00000075] overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-full max-h-full">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form id="change_proxy" method="post" action="#">
                    @csrf
    ...
    <script>
        ...
        function toggleModalAndPopulateForm2(e) {
            const id = e.target.getAttribute('id');
            const groupId = id.replace('change-proxy-', '');
            const form = document.getElementById('change_proxy');
            form.action = `/account/group/change_proxy/${groupId}`;
            toggleModal2(e);
        }

        document.querySelectorAll('[data-modal-toggle="change-proxy-modal"]').forEach(button => {
            button.addEventListener('click', toggleModalAndPopulateForm2);
        });
